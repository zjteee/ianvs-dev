...
class ParadigmBase:
    ...
    def build_paradigm_job(self, paradigm_type):
        ...
        if paradigm_type == ParadigmType.JOINT_INFERENCE.value:
            if getattr(self, "hard_example_mining_mode", None) == "mining-free":
                return JointInference(
                    drafter=self.module_instances.get(ModuleType.DRAFTER.value),
                    verifier=self.module_instances.get(ModuleType.VERIFIER.value),
