...
class JointInference(ParadigmBase):
    ...
    def __init__(self, workspace, **kwargs):
        ...
        self.warmup_samples = max(0, int(kwargs.get("warmup_samples", 0) or 0))
    ...
    def set_config(self):
        ...
        if self.hard_example_mining_mode not in {"self-design", "mining-free"}:
        ...
        elif not required_modules.issubset(set(self.module_instances.keys())):
            raise KeyError(
    ...
    def _inference(self, job):
        ...
        total_samples = len(self.inference_dataset.x)
        warmup_samples = min(self.warmup_samples, total_samples)
        effective_samples = total_samples - warmup_samples

        pbar = tqdm(self.inference_dataset.x, total=total_samples, ncols=100)
        if warmup_samples > 0:
            LOGGER.info(
                "Warmup enabled: running %s samples as warmup (excluded from benchmark), "
                "%s samples remain for evaluation.",
                warmup_samples,
                effective_samples,
            )
