...
class SpeculativeVerifyModel(BaseSpeculativeVerifier):
    ...
    def verify_tokens(
        ...
        if draft_ids:
            ...
            verify_session = self._get_active_verify_session(request, prompt_ids, committed_ids)
            materialized_ids = list(verify_session.get("materialized_ids", []))
            pending_suffix_ids = list(committed_ids[len(materialized_ids):])
            pending_materialization_count = int(len(pending_suffix_ids))
            combined_input_ids = pending_suffix_ids + list(draft_ids)
            ...
            with torch.no_grad():
                verify_outputs = self.model(
                    ...
                    past_key_values=verify_session["past_key_values"],
                    use_cache=True,
                )
            p_logits_for_verify_inputs = []
            if pending_materialization_count > 0:
                pending_offset = pending_materialization_count - 1
                p_logits_for_verify_inputs = [
                    verify_outputs.logits[:, pending_offset + idx, :]
                    for idx in range(len(draft_ids))
                ]
            else:
                prefix_next_logits = verify_session.get("next_logits")
                if prefix_next_logits is None:
                    raise RuntimeError(
                        "Verifier session is missing prefix next_logits for draft verification."
                    )
                p_logits_for_verify_inputs.append(prefix_next_logits)
                p_logits_for_verify_inputs.extend(
                    [
                        verify_outputs.logits[:, idx, :]
                        for idx in range(max(len(draft_ids) - 1, 0))
                    ]
                )
        ...
        return {
            ...
            "decision_state": {
                ...
                "base_cache_length": 0,
                "pending_materialization_count": int(pending_materialization_count),
                "materialized_commit_ids": materialized_ids + pending_suffix_ids + list(accepted_ids),
                "next_logits_after_accept": locals().get("next_logits_after_accept"),
                "verify_past_key_values": _ensure_dynamic_cache(
                    getattr(verify_outputs, "past_key_values", None)
                ),
            },
            "stop": stop,
            "stop_reason": stop_reason,
    ...
    def append_committed(
        ...
        """
        # Commit phase:
        # - reuse the cache produced by the verify forward of this round
        # - crop it back to the trusted materialized committed prefix
        # - do NOT eagerly forward correction / bonus tokens
        #   they remain logical-only and are lazily materialized next round
        ...
        if isinstance(decision_state, dict):
            ...
            verify_past_key_values = decision_state.get("verify_past_key_values")
            next_logits_after_accept = decision_state.get("next_logits_after_accept")
        update_start = time.perf_counter()
        request_id = self._get_request_id(request)
        if verify_past_key_values is not None:
            cache = _ensure_dynamic_cache(verify_past_key_values)
            trusted_cache_length = int(session["prompt_ids"].shape[1]) + int(len(materialized_commit_ids))
            if hasattr(cache, "crop"):
                cache.crop(trusted_cache_length)
            self._verify_sessions[request_id] = {
                "materialized_ids": list(materialized_commit_ids),
                "past_key_values": cache,
                "next_logits": next_logits_after_accept,
            }
        else:
        ...
        crop_ms = (time.perf_counter() - update_start) * 1000.0
